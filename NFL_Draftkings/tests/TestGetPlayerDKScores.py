...
class TestPublicApiMethods(unittest.TestCase):
    ...
    def test_get_scores_name(self):
        data = get_scores(name='Tom Brady', season=2015)
        assert len(data) == 1 and data[0]['name'] == 'Tom Brady'

    def test_get_scores_name_and_week(self):
        data = get_scores(name='Tom Brady', week=1, season=2015)
        assert len(data) == 1 and data[0]['name'] == 'Tom Brady'
    ...
    def test_get_scores_position_and_week_and_name_and_team(self):
        data = get_scores(name='Tom Brady', position='QB', team='NE', week=1, season=2015)
    ...
    def test_get_weekly_scores_name(self):
        data = get_weekly_scores(name='Tom Brady', season=2015)
        assert len(data) == 16 and not [x for x in data if x['name'] != 'Tom Brady']

    def test_get_weekly_scores_name_and_weeks(self):
        data = get_weekly_scores(name='Tom Brady', weeks=[1,2,3], season=2015)
    ...
    def test_get_weekly_scores_position_and_week_and_name_and_team(self):
        data = get_weekly_scores(name='Julian Edelman', position='WR', team='NE', weeks=[1,2,3], season=2015)
